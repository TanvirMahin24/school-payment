...
const TENANT = "coaching";

const syncCoachingStudents = async (updatedSince = null) => {
  try {
    ...
    if (!apiUrl || !apiKey) {
      console.error(
        "COACHING_API_URL or COACHING_API_KEY not configured"
      );
    ...
    if (!response.data.success || !response.data.data) {
      console.error(
        "Invalid response from coaching API:",
        response.data
      );
    ...
    console.log(
      `Coaching student sync completed: ${createdCount} created, ${updatedCount} updated, ${students.length} total`
    );
    return {
      success: true,
      stats: {
    ...
    console.error("Error syncing coaching students:", error.message);
    if (error.response) {
...
module.exports = { syncCoachingStudents };
