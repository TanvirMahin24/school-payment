...
const syncSchoolStudents = async (updatedSince = null) => {
  try {
    ...
    if (!apiUrl || !apiKey) {
      console.error(
        "SCHOOL_API_URL or SCHOOL_API_KEY not configured"
      );
    ...
    if (!response.data.success || !response.data.data) {
      console.error(
        "Invalid response from school API:",
        response.data
      );
    ...
    console.log(
      `School student sync completed: ${createdCount} created, ${updatedCount} updated, ${students.length} total`
    );
    return {
      success: true,
      stats: {
    ...
    console.error("Error syncing school students:", error.message);
    if (error.response) {
...
module.exports = { syncSchoolStudents };
